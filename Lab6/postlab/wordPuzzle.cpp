...
int main(int argc, char ** argv) {
  ...
  int count = 0;
    ...
    int wordsFound = 0;
    string dir;
    vector<string> foundWords; //removes printing to screen during looping
    timer myTimer;
    myTimer.start();
	...
	for(int d = 0; d < 8; d++) {
	  for(int w = 3; w < 23; w++) {
		...
		dir = "NW (";
	      ...
	      rr << i;
	      cc << j;
	      fin += dir + rr.str() + "," + cc.str() + "):\t" + word;
	      foundWords.push_back(fin);
	    }
	  }
	}
      }
   ...
   myTimer.stop();
    int totalTime = myTimer.getTime();
    for(int i = 0; i < foundWords.size(); i++) {
      cout << foundWords[i] << endl;
    }
    cout << wordsFound << " words found" << endl;
    cout <<"The total time is: " << totalTime << " seconds " << endl;
    cout << totalTime*1000 << endl; //so it can be read by averagetime.sh
