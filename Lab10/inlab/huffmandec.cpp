...
// I altered the skeleton code for the lab 10 in-lab.  It uses
...
void huffmanTree(huffmanNode* n, string prefix, char let) {
 if(prefix.length()==0) {
    n->character = let;
  }
  if(prefix[0] == '0') {
    ...
    huffmanTree(n->left, prefix.substr(1, prefix.length()-1), let);
  }
  else if(prefix[0] == '1') {
    if(n->right == NULL) {
      n->right = new huffmanNode(0, '\0');
    }
    huffmanTree(n->right, prefix.substr(1, prefix.length()-1), let);
  }
...
int main (int argc, char **argv) {
	...
	string pcode = string(buffer);
	huffmanTree(n,pcode, first); 
	...
	if(myNode->left != NULL && bit == '0') {
	  myNode = myNode->left;
	}
	if(myNode->right != NULL && bit == '1') {
	  myNode = myNode->right;
	}
